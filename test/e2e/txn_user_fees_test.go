...
func TestUserFees_HappyPath(t *testing.T) {
	...
	t.Logf("Initial submission status: %v, error: %v", st.Code, st.Error)

	sim.StepN(20)

	// Query the transaction status to see what happened
	record := sim.QueryTransaction(st.TxID, nil)
	t.Logf("Transaction status after stepping: %v, error: %v", record.Status, record.Error)
	if record.Signatures != nil {
		t.Logf("Signature count: %d", record.Signatures.Total)
		for i, sigRecord := range record.Signatures.Records {
			t.Logf("  Signature set %d: Account=%v, Total=%d", i, sigRecord.Account, sigRecord.Signatures.Total)
		}
	}

	// Step more to see if it eventually completes
	sim.StepN(50)
	record = sim.QueryTransaction(st.TxID, nil)
	t.Logf("Transaction status after more stepping: %v, error: %v", record.Status, record.Error)

	if record.Error != nil {
		t.Fatalf("Transaction failed with error: %v", record.Error.Message)
	}
